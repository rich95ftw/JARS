...
def plot_geometry():
    ...
    try:
        ...
        # We need to draw the canvas once to be able to get the renderer,
        # which is needed to calculate the bounding box of the text.
        fig.canvas.draw()
        ...
        # --- Logic to keep annotations inside the plot ---
        all_x = [tx_pos.x, jam_pos.x, rx_pos.x]
        all_y = [tx_pos.y, jam_pos.y, rx_pos.y]
        x_center = sum(all_x) / len(all_x)
        y_center = sum(all_y) / len(all_y)

        def get_annotation_params(x, y):
            ha = 'left' if x < x_center else 'right'
            x_offset = 15 if x < x_center else -15
            va = 'bottom' if y < y_center else 'top'
            y_offset = 15 if y < y_center else -15
            return ha, va, x_offset, y_offset

        ha, va, xo, yo = get_annotation_params(tx_pos.x, tx_pos.y)
        ax.annotate(f"{tx.power_dbm:.1f} dBm",
                    xy=(tx_pos.x, tx_pos.y), xytext=(xo, yo),
                    textcoords="offset points", ha=ha, va=va,
                    bbox=dict(boxstyle="round,pad=0.4", fc="lightblue", ec="none", alpha=0.8),
                    arrowprops=dict(arrowstyle="->", connectionstyle="arc3,rad=0.3", color='blue'))

        ha, va, xo, yo = get_annotation_params(jam_pos.x, jam_pos.y)
        ax.annotate(f"{jammer.power_dbm:.1f} dBm",
                    xy=(jam_pos.x, jam_pos.y), xytext=(xo, yo),
                    textcoords="offset points", ha=ha, va=va,
                    bbox=dict(boxstyle="round,pad=0.4", fc="#ffcccb", ec="none", alpha=0.8),
        ...
        # --- REFACTORED: Robust, non-overlapping corner annotations ---
        # We now stack the annotations dynamically.

        # 1. J/S Ratio Annotation
        t1 = ax.text(0.05, 0.95, f"J/S = {j_s:.1f} dB",
        ...
        # Get the renderer to calculate text bounding box
        renderer = fig.canvas.get_renderer()
        bbox_t1 = t1.get_window_extent(renderer=renderer)

        # Convert bounding box to axes coordinates
        bbox_t1_axes = bbox_t1.transformed(ax.transAxes.inverted())

        # 2. Jamming Success/Failure Annotation
        # The vertical position of this text is now dynamically calculated
        # to be just below the first text annotation.
        threshold = float(threshold_var.get())
        ...
        ax.text(0.05, bbox_t1_axes.y0 - 0.08, status_text,  # y0 is the bottom of t1
                transform=ax.transAxes,
                fontsize=8,
                fontweight='bold',
                verticalalignment='top',
...
root = tk.Tk()
root.title("JARS - Jamming Analysis and Radio Simulation")
